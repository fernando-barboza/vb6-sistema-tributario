...
Begin VB.Form frmCadGrupoAssunto 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Grupos de Assunto"
   ClientHeight    =   3960
   ClientLeft      =   3615
   ClientTop       =   2685
   ClientWidth     =   5835
   HelpContextID   =   115
   Icon            =   "CadGrupoAssunto.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   ...
   Begin TabDlg.SSTab tab_3dPasta 
      ...
      TabsPerRow      =   1
      TabHeight       =   520
      TabCaption(0)   =   "Grupos de Assunto"
      TabPicture(0)   =   "CadGrupoAssunto.frx":1042
      Tab(0).ControlEnabled=   -1  'True
      Tab(0).Control(0)=   "lblstrDescricao"
      Tab(0).Control(0).Enabled=   0   'False
      Tab(0).Control(1)=   "lblstrCodGrpAssuntos"
      Tab(0).Control(1).Enabled=   0   'False
      Tab(0).Control(2)=   "tdb_Lista"
      Tab(0).Control(2).Enabled=   0   'False
      Tab(0).Control(3)=   "txtstrDescricao"
      Tab(0).Control(3).Enabled=   0   'False
      Tab(0).Control(4)=   "txtstrCodGrpAssuntos"
      Tab(0).Control(4).Enabled=   0   'False
      Tab(0).ControlCount=   5
      Begin VB.TextBox txtstrCodGrpAssuntos 
         Height          =   285
         Left            =   900
         MaxLength       =   10
      ...
      Begin TrueOleDBGrid70.TDBGrid tdb_Lista 
         ...
         Columns(1).DataField=   "strCodGrpAssuntos"
         ...
         Splits(0)._ColumnProps(13)=   "Column(2).Width=7885"
         Splits(0)._ColumnProps(14)=   "Column(2).DividerColor=0"
         Splits(0)._ColumnProps(15)=   "Column(2)._WidthInPix=7805"
      ...
      End
      Begin VB.Label lblstrCodGrpAssuntos 
         AutoSize        =   -1  'True
         Caption         =   "Código"
         Height          =   195
         Left            =   345
   ...
   End
End
Attribute VB_Name = "frmCadGrupoAssunto"
...
Private Function strQuery(Optional blnAplicar As Boolean) As String

'******************************************************************************************
' Data: 27/03/2003
' Alteração: - Substituição do comando CAST do SQL Server pela função gstrCONVERT.
...
'******************************************************************************************
    ...
    Else
        strSql = strSql & "SELECT PKId, strCodGrpAssuntos, strDescricao FROM "
        strSql = strSql & gstrGrupoAssunto & " "
        strSql = strSql & "ORDER BY " & gstrCONVERT(CDT_INT, "strCodGrpAssuntos")
    End If
    strQuery = strSql
End Function

Private Sub Form_Activate()
    gintCodSeguranca = 438
...
Private Sub Form_Load()
    VerificaListaAutomatica gstrGrupoAssunto, tdb_Lista, strQuery
...
Private Sub tdb_Lista_RowColChange(LastRow As Variant, ByVal LastCol As Integer)
    With tdb_Lista
        If (Not .EOF And Not .BOF) And mblnClickOk Then
            ...
            LeDaTabelaParaObj gstrGrupoAssunto, Me
            ...
            strCodigoAtual = txtstrCodGrpAssuntos.Text
...
Public Sub MantemForm(ByVal strModoOperacao As String)
    ...
    ToolBarGeral strModoOperacao, gstrGrupoAssunto, mblnAlterando, tdb_Lista, Me, _
                 mobjAux, strQuery, strQuery(True), rptGrupoDeAssunto, strQueryRelatorio
End Sub

Private Sub txtstrCodGrpAssuntos_GotFocus()
    gstrProximoCodigo txtstrCodGrpAssuntos, gstrGrupoAssunto, "strCodGrpAssuntos", gintCodSeguranca
    MarcaCampo txtstrCodGrpAssuntos
End Sub

Private Sub txtstrCodGrpAssuntos_KeyPress(KeyAscii As Integer)
    CaracterValido KeyAscii, "N", txtstrCodGrpAssuntos
End Sub

Private Sub txtstrCodGrpAssuntos_LostFocus()
    If Len(txtstrCodGrpAssuntos) > 0 Then txtstrCodGrpAssuntos = Val(txtstrCodGrpAssuntos)
End Sub
...
Private Function strQueryRelatorio() As String
    Dim strSql  As String
    strSql = ""
    strSql = strSql & "SELECT strCodGrpAssuntos, strDescricao "
    strSql = strSql & "FROM " & gstrGrupoAssunto & " "
    strSql = strSql & "ORDER BY strCodGrpAssuntos, strDescricao "
    strQueryRelatorio = strSql
End Function

Private Function blnDadosOk() As Boolean
    
    If Val(txtstrCodGrpAssuntos.Text) = 0 Then
        ExibeMensagem "Preencha corretamente o campo código!"
        txtstrCodGrpAssuntos.SetFocus
        Exit Function
    ...
    If Not mblnAlterando Or (mblnAlterando And UCase$(strCodigoAtual) <> UCase$(txtstrCodGrpAssuntos.Text)) Then

ProximoCodigo:

        If gblnExisteCodigo(1, gstrGrupoAssunto, "strCodGrpAssuntos", "'" & Val(txtstrCodGrpAssuntos.Text) & "'") Then
            strCodigo = (gstrProximoCodigo(txtstrCodGrpAssuntos, gstrGrupoAssunto, "strCodGrpAssuntos", gintCodSeguranca, , , , True))
            If MsgBox("O código informado já se encontra cadastrado. Deseja usar o código " & strCodigo & "?", vbYesNo + vbQuestion) = vbNo Then
                txtstrCodGrpAssuntos.SetFocus
                Exit Function
            Else
                txtstrCodGrpAssuntos.Text = strCodigo
                GoTo ProximoCodigo
            End If
        End If
    End If
    ...
    If Not mblnAlterando Or (mblnAlterando And UCase$(txtstrDescricao.Text) <> UCase$(strDescricaoAtual)) Then
        ...
        If gblnExisteCodigo(1, gstrGrupoAssunto, "strDescricao", "'" & Trim(txtstrDescricao.Text) & "'") Then
