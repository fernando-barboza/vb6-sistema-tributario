...
Begin VB.Form frmCadAtividadesBasicas 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Atividades Básicas"
   ClientHeight    =   4515
   ClientLeft      =   1485
   ClientTop       =   1935
   ClientWidth     =   5685
   ...
   Begin TabDlg.SSTab tab_3dPasta 
      ...
      TabsPerRow      =   1
      TabHeight       =   520
      TabCaption(0)   =   "Atividades Básicas"
      TabPicture(0)   =   "frmCadAtividadesBasicas.frx":0000
      Tab(0).ControlEnabled=   -1  'True
      Tab(0).Control(0)=   "lblstrDescricao"
      Tab(0).Control(0).Enabled=   0   'False
      Tab(0).Control(1)=   "lblintCodigo"
      Tab(0).Control(1).Enabled=   0   'False
      Tab(0).Control(2)=   "tdb_AtividadesBasicas"
      ...
      Begin VB.TextBox txtintCodigo 
         ...
         Width           =   870
      End
      Begin TrueOleDBGrid70.TDBGrid tdb_AtividadesBasicas 
         Height          =   2865
         Left            =   135
         TabIndex        =   4
         Top             =   1305
         ...
         Splits(0)._ColumnProps(12)=   "Column(2).Width=6376"
         Splits(0)._ColumnProps(13)=   "Column(2).DividerColor=0"
         Splits(0)._ColumnProps(14)=   "Column(2)._WidthInPix=6297"
   ...
   End
End
Attribute VB_Name = "frmCadAtividadesBasicas"
...
Private Sub tdb_AtividadesBasicas_Click()
    blnPrimeiraVez = True
    'mblnClickOk = False
    If glngQtdLinhaTDBGrid(tdb_AtividadesBasicas) = 1 Then
        tdb_AtividadesBasicas_RowColChange 0, 0
    End If
End Sub

Private Function strQueryAtividadesBasicas() As String
...
strQueryAtividadesBasicas = strsql

End Function

Public Sub MantemForm(ByVal strModoOperacao As String)
Dim strsql As String
strsql = strQueryAtividadesBasicas
If strModoOperacao = UCase("IMPRIMIR") Then
    ToolBarGeral strModoOperacao, gstrAtividadeBasica, blnAlterando, tdb_AtividadesBasicas, Me, mobjAux, strsql, , rptatividadebasica, strQueryAtividadesBasicas
    Exit Sub
End If

Select Case UCase(strModoOperacao)
    Case Is = UCase(gstrLocalizar)
        ToolBarGeral strModoOperacao, gstrAtividadeBasica, False, tdb_AtividadesBasicas, Me, mobjAux, strQueryAtividadesBasicas
    Case Is = UCase(gstrNovo)
        TrocaCorObjeto txtintCodigo, False
        LimpaObjeto Me
        blnPrimeiraVez = False
        blnAlterando = False
        gstrProximoCodigo txtintCodigo, gstrAtividadeBasica, "intCodigo", gintCodSeguranca
        txtintCodigo.SetFocus
    Case Is = UCase(gstrFechar)
        Unload Me
    Case Is = UCase(gstrSalvar)
        If blnDadosOk Then
           ToolBarGeral strModoOperacao, gstrAtividadeBasica, blnAlterando, tdb_AtividadesBasicas, _
                        Me, mobjAux, strQueryAtividadesBasicas
           If gblnCancelarInclusao = False Then
              ToolBarGeral UCase(gstrLocalizar), gstrAtividadeBasica, False, tdb_AtividadesBasicas, Me, mobjAux, _
                           strQueryAtividadesBasicas
           End If
        End If
    Case Is = UCase(gstrDeletar)
         ToolBarGeral strModoOperacao, gstrAtividadeBasica, blnAlterando, tdb_AtividadesBasicas, _
                      Me, mobjAux, strQueryAtividadesBasicas, strQueryAplicar
         If gblnCancelarInclusao = False Then
            ToolBarGeral UCase(gstrLocalizar), gstrAtividadeBasica, False, tdb_AtividadesBasicas, Me, mobjAux, _
                         strQueryAtividadesBasicas
         End If
...
Private Sub tdb_AtividadesBasicas_DblClick()
    MantemForm gstrAplicar
End Sub

Private Sub tdb_AtividadesBasicas_FilterChange()
    gblnFilraCampos tdb_AtividadesBasicas
End Sub

Private Sub tdb_AtividadesBasicas_HeadClick(ByVal ColIndex As Integer)
    gOrdenaGrid tdb_AtividadesBasicas, ColIndex
End Sub

Private Sub tdb_AtividadesBasicas_KeyDown(KeyCode As Integer, Shift As Integer)
    mblnClickOk = False
End Sub

Private Sub tdb_AtividadesBasicas_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    mblnClickOk = True
End Sub

Private Sub tdb_AtividadesBasicas_RowColChange(LastRow As Variant, ByVal LastCol As Integer)
With tdb_AtividadesBasicas
If (Not .EOF And Not .BOF) And mblnClickOk Then
    If blnPrimeiraVez Then
        blnAlterando = True
        txtPkid = tdb_AtividadesBasicas.Columns("Pkid")
        TrocaCorObjeto txtintCodigo, True
        LeDaTabelaParaObj gstrAtividadeBasica, Me
        strCodigoAtual = tdb_AtividadesBasicas.Columns("Código")
        strDescriAtual = tdb_AtividadesBasicas.Columns("Descrição")
    End If
End If
End With
End Sub
...
ProximoCodigo:
        ...
        If gblnExisteCodigo(1, gstrAtividadeBasica, "intCodigo", "'" & txtintCodigo.Text & "'") Then
            strCodigo = (gstrProximoCodigo(txtintCodigo, gstrAtividadeBasica, "intCodigo", gintCodSeguranca, , , , True))
    ...
    If Not blnAlterando Or (blnAlterando And UCase$(txtstrDescricao.Text) <> UCase$(strDescriAtual)) Then
        ...
        If gblnExisteCodigo(1, gstrAtividadeBasica, "strDescricao", "'" & txtstrDescricao.Text & "'") Then
...
strsql = strsql & " FROM " & gstrAtividadeBasica
