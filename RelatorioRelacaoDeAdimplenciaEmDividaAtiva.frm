...
Begin VB.Form frmRelatorioRelacaoDeAdimplenciaEmDividaAtiva 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Relação de Adimplência em Dívida Ativa"
   ClientHeight    =   2310
   ClientLeft      =   75
   ClientTop       =   1470
   ClientWidth     =   7530
   Icon            =   "RelatorioRelacaoDeAdimplenciaEmDividaAtiva.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin TabDlg.SSTab tab_3dPasta 
      ...
      TabsPerRow      =   1
      TabHeight       =   520
      TabCaption(0)   =   "Relação de Adimplência em Dívida Ativa"
      TabPicture(0)   =   "RelatorioRelacaoDeAdimplenciaEmDividaAtiva.frx":1042
   ...
   End
End
Attribute VB_Name = "frmRelatorioRelacaoDeAdimplenciaEmDividaAtiva"
...
Dim adoResultado   As ADODB.Recordset
    ...
    If UCase(strModoOperacao) = UCase(gstrImprimir) Then
        ...
        If chk_Selecionar.Value = 0 Then
            ...
            End If
        End If
            ImprimeRelatorio rptRelacaoDeAdimplenciaEmDividaAtiva, strQueryRelatorio
        End If
...
Dim strSql As String
    ...
    strSql = ""
    strSql = strSql & " SELECT DISTINCT CO.strNome "
    strSql = strSql & " FROM "
...
'    strSql = strSql & gstrContribuinte & " AS CO "
    ...
    End If
    strSql = strSql & " AND DDA.bytSituacao = 0 " 'Quitado
    
    strSql = strSql & " AND DA.intContribuinte NOT IN(SELECT DISTINCT DA.intContribuinte FROM "
'    strSql = strSql & gstrDividaAtiva & " AS DA, "
    strSql = strSql & gstrDividaAtiva & " DA, "
'    strSql = strSql & gstrDetalheDividaAtiva & " AS DDA "
    strSql = strSql & gstrDetalheDividaAtiva & " DDA "
    strSql = strSql & " WHERE DDA.intDividaAtiva = DA.PKId AND bytSituacao <> 0) " 'somente se quitou todas as parcelas da dívida ativa
    
    strSql = strSql & " AND DA.intContribuinte NOT IN(SELECT DISTINCT DA.intContribuinte FROM "
'    strSql = strSql & gstrLancamentoCalculo & " AS LC, "
    strSql = strSql & gstrLancamentoCalculo & " LC, "
'    strSql = strSql & gstrParcelaReceita & " AS PR "
    strSql = strSql & gstrParcelaReceita & " PR "
    strSql = strSql & " WHERE PR.intLancamentoCalculo = LC.PKId AND bytAtiva = 0) " 'não pode estar aberto (bytativa=1) na parcela receita
