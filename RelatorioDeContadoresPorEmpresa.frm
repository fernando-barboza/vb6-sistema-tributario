...
Begin VB.Form frmRelatorioDeContadoresPorEmpresa 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Relação de Contadores por Empresa"
   ClientHeight    =   2295
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   7440
   Icon            =   "RelatorioDeContadoresPorEmpresa.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin TabDlg.SSTab tab_3dPasta 
      ...
      TabsPerRow      =   1
      TabHeight       =   520
      TabCaption(0)   =   "Relação de Contadores por Empresa"
      TabPicture(0)   =   "RelatorioDeContadoresPorEmpresa.frx":1042
   ...
   End
End
Attribute VB_Name = "frmRelatorioDeContadoresPorEmpresa"
...
Dim adoResultado   As adodb.Recordset
    ...
    If UCase(strModoOperacao) = UCase(gstrImprimir) Then
        ...
        If chk_Selecionar.Value = 0 Then
            ...
            End If
        End If
            ImprimeRelatorio rptRelatorioDeContadoresPorEmpresa, strQuerryRelatorio
        End If
    End If
...
Dim strSql As String
    ...
    strSql = ""
    strSql = strSql & " SELECT "
    strSql = strSql & " B.intContribuinte, A.strNome, A.strCNPJCPF, B.intContador "
    
    strSql = strSql & " FROM "
    strSql = strSql & gstrContribuinte & " A, "
    strSql = strSql & gstrEconomico & " B, "
    strSql = strSql & gstrContador & " C "
    
    strSql = strSql & " WHERE "
    strSql = strSql & " B.intContribuinte = A.PKId "
    strSql = strSql & " AND B.intContador IS NOT NULL "
    strSql = strSql & " AND B.intContador = C.PKId "
    
    If chk_Selecionar.Value = 0 Then
        strSql = strSql & " AND C.intContribuinte BETWEEN " & CCInicial & " AND " & CCFinal
    End If
    strSql = strSql & " ORDER BY B.intContador "
