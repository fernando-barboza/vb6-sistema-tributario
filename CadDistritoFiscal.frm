...
Begin VB.Form frmCadDistritoFiscal 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Distritos Fiscais"
   ClientHeight    =   5175
   ClientLeft      =   2940
   ClientTop       =   4455
   ClientWidth     =   6480
   HelpContextID   =   19
   Icon            =   "CadDistritoFiscal.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin TabDlg.SSTab tab_3dPasta 
      ...
      TabsPerRow      =   1
      TabHeight       =   529
      TabCaption(0)   =   "Distritos Fiscais"
      TabPicture(0)   =   "CadDistritoFiscal.frx":1042
      Tab(0).ControlEnabled=   -1  'True
      Tab(0).Control(0)=   "lblstrDescricao"
      Tab(0).Control(0).Enabled=   0   'False
      Tab(0).Control(1)=   "lblintCodigo"
      Tab(0).Control(1).Enabled=   0   'False
      Tab(0).Control(2)=   "tdb_DistritoFiscal"
      ...
      Begin VB.TextBox txtstrDescricao 
         ...
         Width           =   5085
      End
      Begin TrueOleDBGrid70.TDBGrid tdb_DistritoFiscal 
         Height          =   3525
         Left            =   135
         TabIndex        =   6
         Top             =   1260
         ...
         _StyleDefs(7)   =   "CaptionStyle:id=4,.parent=2,.namedParent=37,.alignment=1"
   ...
   End
End
Attribute VB_Name = "frmCadDistritoFiscal"
...
Private Sub Form_Load()
    ...
    mblnAlterando = False
    VerificaListaAutomatica gstrDistritoFiscal, tdb_DistritoFiscal, strQuery
...
Private Sub tdb_DistritoFiscal_Click()
    mblnAlterando = False
    If glngQtdLinhaTDBGrid(tdb_DistritoFiscal) = 1 Then
        tdb_DistritoFiscal_RowColChange 0, 0
    End If
    
End Sub

Private Sub tdb_DistritoFiscal_DblClick()
    MantemForm gstrAplicar
End Sub

Private Sub tdb_DistritoFiscal_FilterChange()
    'mblnAlterando = True
    gblnFilraCampos tdb_DistritoFiscal
End Sub
...
Private Function strQuery() As String
    ...
    strSql = strSql & gstrDistritoFiscal
...
Private Sub tdb_DistritoFiscal_HeadClick(ByVal ColIndex As Integer)
    gOrdenaGrid tdb_DistritoFiscal, ColIndex
End Sub

Private Sub tdb_DistritoFiscal_KeyDown(KeyCode As Integer, Shift As Integer)
  mblnAlterando = True
End Sub

Private Sub tdb_DistritoFiscal_KeyPress(KeyAscii As Integer)
    Select Case tdb_DistritoFiscal.Col
        Case 1
            CaracterValido KeyAscii, "N", tdb_DistritoFiscal
        Case Else
            CaracterValido KeyAscii, "A", tdb_DistritoFiscal
    End Select
    mblnAlterando = True
End Sub

Private Sub tdb_DistritoFiscal_RowColChange(LastRow As Variant, ByVal LastCol As Integer)
    With tdb_DistritoFiscal
        If Not .EOF And Not .BOF Then
            If mblnAlterando = False Then
               'mblnAlterando = True
               txtPKID.Text = .Columns("PKID").Value
               LeDaTabelaParaObj gstrDistritoFiscal, Me
               gCorLinhaSelecionada tdb_DistritoFiscal
...
Public Sub MantemForm(ByVal strModoOperacao As String)
    ...
    '    ToolBarGeral strModoOperacao, gstrDistritoFiscal, mblnAlterando, tdb_DistritoFiscal, Me, mobjAux, strSql, , rptdistritosfiscais, strQuery
    '    Exit Sub
    'End If
...
Private Function blnDadosOk() As Boolean
    ...
    If mblnAlterando = False Or (mblnAlterando = True And Trim(txtintCodigo.Text) <> tdb_DistritoFiscal.Columns(1).Text) Then
       If gblnExisteCodigo(1, gstrDistritoFiscal, "intCodigo", Trim(txtintCodigo.Text)) And _
       Trim(txtintCodigo.Text) Then
    ...
    If mblnAlterando = False Or (mblnAlterando = True And Trim(txtstrDescricao.Text) <> tdb_DistritoFiscal.Columns(2).Text) Then
       If gblnExisteCodigo(1, gstrDistritoFiscal, "strDescricao", "'" & Trim(txtstrDescricao.Text) & "'") Then
...
Private Sub txtintCodigo_GotFocus()
    ...
    gstrProximoCodigo txtintCodigo, gstrDistritoFiscal, "intCodigo", gintCodSeguranca
End Sub
