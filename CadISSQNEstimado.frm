...
Begin VB.Form frmcadISSQNEstimado 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lançamento de ISSQN Estimado"
   ClientHeight    =   7080
   ClientLeft      =   1920
   ClientTop       =   2445
   ClientWidth     =   8685
   Icon            =   "CadISSQNEstimado.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin TabDlg.SSTab tab_3dPasta 
      ...
      Style           =   1
      TabHeight       =   520
      TabCaption(0)   =   "Lançamento de ISSQN Estimado"
      TabPicture(0)   =   "CadISSQNEstimado.frx":1042
      ...
      TabPicture(1)   =   "CadISSQNEstimado.frx":105E
      Tab(1).ControlEnabled=   0   'False
      Tab(1).Control(0)=   "tdb_Lista"
      Tab(1).Control(0).Enabled=   0   'False
      Tab(1).Control(1)=   "tdb_Atividades"
      Tab(1).Control(1).Enabled=   0   'False
      Tab(1).Control(2)=   "fra_frame"
      Tab(1).Control(2).Enabled=   0   'False
      Tab(1).ControlCount=   3
      TabCaption(2)   =   "Emissão de Guias de Arrecadação"
      TabPicture(2)   =   "CadISSQNEstimado.frx":107A
      ...
      Begin VB.Frame fra_frame 
         ...
         Begin VB.TextBox txt_intExercício 
            ...
            Width           =   705
         End
         Begin VB.TextBox txt_AliquotaEstimativa 
            Alignment       =   1  'Right Justify
            Height          =   285
            Left            =   6045
            MaxLength       =   3
         ...
         Begin VB.Label lbl_dtmDataPagamento 
            ...
            Width           =   1500
         End
         Begin VB.Label lbl_AliquotaEstimativa 
            AutoSize        =   -1  'True
            Caption         =   "Alíquota de Estimativa"
            Height          =   195
            Left            =   4305
            TabIndex        =   57
            Top             =   285
      ...
      Begin TrueOleDBGrid70.TDBGrid tdb_Proprietarios 
         ...
         Splits(0)._ColumnProps(12)=   "Column(2).Width=10901"
         Splits(0)._ColumnProps(13)=   "Column(2).DividerColor=0"
         Splits(0)._ColumnProps(14)=   "Column(2)._WidthInPix=10821"
      ...
      Begin TrueOleDBGrid70.TDBGrid tdb_Atividades 
         ...
         Splits(0)._ColumnProps(14)=   "Column(2).Width=12277"
         Splits(0)._ColumnProps(15)=   "Column(2).DividerColor=0"
         Splits(0)._ColumnProps(16)=   "Column(2)._WidthInPix=12197"
   ...
   End
End
Attribute VB_Name = "frmcadISSQNEstimado"
...
Private Sub dbc_strInscricaoCadastral_KeyPress(KeyAscii As Integer)
    CaracterValido KeyAscii, "A", dbc_strInscricaoCadastral
End Sub

Private Sub Form_Activate()
    gintCodSeguranca = 669
...
Private Sub Form_Load()
    ...
    'LeDaTabelaParaObj gstrImobiliario, , strQueryEconomico(0)
...
Private Sub txt_AliquotaEstimativa_GotFocus()
    MarcaCampo txt_AliquotaEstimativa
End Sub

Private Sub txt_AliquotaEstimativa_KeyPress(KeyAscii As Integer)
    CaracterValido KeyAscii, "V", txt_AliquotaEstimativa
End Sub
    ...
    If UCase(strModoOperacao) = UCase(gstrImprimir) Then
        If blnDadosGuiaOK = True Then
            ...
            strSql = gstrQueryRelatorioGuiaDeArrecadacao(blnExiteLancamento, strInscricao, strInscricao, _
                                txt_intExercício.Text, dbc_intComposicaoDaReceita.BoundText, , , _
            ...
            End If
        End If
    ElseIf strModoOperacao = gstrCalcularReajuste Then
        CalculoLancamento
...
'******************************************************************************************
    ...
    Dim dblValorAliquota  As String
    Dim strInscricao      As String
    ...
    If blnDadosOk Then
        strInscricao = Trim(Left(dbc_strInscricaoCadastral.Text, InStr(1, dbc_strInscricaoCadastral.Text, _
                                "-", vbTextCompare) - 1))
'        strSQL = "sp_CalculoParaUsuario '" & strInscricao & "','" & strPKId & "',2," & _
'                 dbc_intComposicaoDaReceita.BoundText
        strSql = gstrStoredProcedure("sp_CalculoParaUsuario", "'" & strInscricao & "','" & strPKId & "',2," & _
                 dbc_intComposicaoDaReceita.BoundText & ", 0, 0, 0, '" & _
                 IIf((bytDBType = EDatabases.Oracle), ":V_dblValor", "@dblValor OUTPUT") & "'")
        Set gobjBanco = New clsBanco
'        If gobjBanco.CriaADO(strSQL, 10, adoResultado) Then
        If gobjBanco.ExecuteStoredProcedure(strSql, 10, adoResultado, adoParameters) Then
'            If Not (adoResultado.BOF And adoResultado.EOF) Then
            If Not (adoParameters Is Nothing) Then
'                dblValorAliquota = (gstrConvVrDoSql(adoResultado!dblValorAparcelar) * gstrConvVrDoSql(txt_AliquotaEstimativa) / 100)
                dblValorAliquota = (gstrConvVrDoSql(adoParameters("dblValorAparcelar")) * gstrConvVrDoSql(txt_AliquotaEstimativa) / 100)
                'Mostra Valores para Usuário
'                strSQL = "Confirma o cálculo de " & gstrConvVrDoSql(dblValorAliquota) & _
...
'                        (Val(txt_intParcelaFinal) - Val(txt_intParcelaInicial) + 1) & " parcela(s) ?"
                ...
                If MsgBox(strSql, vbYesNo, "Tributário") = vbYes Then
                    ...
                    If Not gBlnVerificaLancamentos(txt_intExercício.Text, dbc_intComposicaoDaReceita.BoundText, _
                        ...
                        Exit Sub
                    End If
'                    strSQL = strLancamentos(dblValorAliquota, adoResultado!dblValorNaoParcelado)
                    strSql = strLancamentos(dblValorAliquota, adoParameters("dblValorNaoParcelado"))
...
'******************************************************************************************
    ...
    Dim strSql       As String
    Dim strInscricao As String
    strInscricao = Trim(Left(dbc_strInscricaoCadastral.Text, InStr(1, _
                        dbc_strInscricaoCadastral.Text, "-", vbTextCompare) - 1))
...
'            Val(txt_intIntervalo.Text) & ",3," & Val(dbc_intOcorrencia.BoundText) & ",2," & glngCodUsr & _
'            ",0,0," & gstrConvVrParaSql(txt_AliquotaEstimativa.Text)
...
Private Sub tab_3dPasta_Click(PreviousTab As Integer)
    If tab_3DPasta.Tab = 1 Then txt_intExercício.SetFocus
End Sub

'==Para Efetuar Cálculo==
Private Function blnDadosOk() As Boolean
    Dim i As Integer
    If dbc_strInscricaoCadastral.MatchedWithList = False Then
    ...
    If txt_intExercício.Text = "" Then
        ...
        Exit Function
    End If
    If txt_AliquotaEstimativa.Text = "" Then
        ExibeMensagem "O campo " & lbl_AliquotaEstimativa.Caption & " não pode ser nulo."
        tab_3DPasta.Tab = 1
        txt_AliquotaEstimativa.SetFocus
        Exit Function
    End If
